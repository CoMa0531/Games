...
public class BlockGame {
	...
	static class MyFrame extends JFrame {
		...
		static class MyPanel extends JPanel { // VANVAS for Draw!
			...
			private void drawUI(Graphics2D g2d) {
				...
				for (int i = 0; i < BLOCK_ROWS; i++) {
					...
					// draw ball
					g2d.setColor(Color.YELLOW);
					g2d.fillOval(ball.x, ball.y, BALL_WIDTH, BALL_HEIGHT); // 원 그리는 함수, 가로 세로가 다르면 타원이 됨.
		...
		public MyFrame(String title) {
			...
			setKeyListener(); // bar가 움직이게 하는 메소드
			startTimer();
		...
		public void initData() {
			for (int i = 0; i < BLOCK_ROWS; i++) {
				for (int j = 0; j < BLOCK_COLUMS; j++) {
					...
					blocks[i][j].y = 100 + BLOCK_HEIGHT * i + BLOCK_GAP * i; // 상단 score라든지 여백을 줘야 함.
		...
		public void checkCollision() {
			if (dir == 0) { // 0 : Up-Right
				...
				if (ball.getBottomCenter().y >= bar.y) { // bar
				...
				if (ball.y > CANVAS_HEIGHT - BALL_HEIGHT - BALL_HEIGHT) { // war bottom
					dir = 2;
				}
				if (ball.getBottomCenter().y >= bar.y) { // bar
					if (duplRect(new Rectangle(ball.x, ball.y, ball.width, ball.height),
